...
Zero testo aggiuntivo. Solo l'array.`
    ...
    return { results: finalResults, filters: filtri, ai_debug: { ...aiDebug, category_variants: categoryVariants, fallback_city_only: usedFallbackCityOnly } }
  ...
  try {
    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        response_format: { type: 'json_object' },
        messages: [
          { role: 'system', content: `Sei un esperto di categorie business italiane. Data una query, estrai la città e genera 6-10 sottocategorie STRETTAMENTE correlate al topic della query. MAI generare categorie di settori diversi da quello richiesto. Esempi: "comunicazione Milano" → ["agenzie di comunicazione","agenzie PR","uffici stampa","social media agency","agenzie pubblicitarie","agenzie grafiche","copywriter","uffici comunicazione"]. "cibo Roma" → ["ristoranti","pizzerie","trattorie","sushi","bar e caffetterie","paninerie","rosticcerie"]. "edilizia Torino" → ["imprese edili","imprese di costruzioni","ristrutturazioni","impianti elettrici","impianti idraulici","serramenti"]. "informatica Milano" → ["software house","web agency","assistenza informatica","sviluppo app","consulenza IT","cybersecurity"]. Restituisci SOLO JSON valido: { "city": "...", "subcategories": ["..."] }` },
    ...
    const GENERIC_SUB_RE = /\b(aziend|attivit|impres|negoz|serviz|professionist|consulenz|soluzion|fornitor|commercial|shop|store|agenzi[ae])\b/i
